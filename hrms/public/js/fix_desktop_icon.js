...
// Ensure frappe.utils is available before overriding
(function() {
	// Wait for frappe.utils to be fully initialized
	if (typeof frappe === 'undefined' || !frappe.utils) {
		// If not ready, wait for it
		if (typeof frappe !== 'undefined' && frappe.ready) {
			frappe.ready(function() {
				overrideGetDesktopIcon();
			});
		} else {
	...
	function overrideGetDesktopIcon() {
		frappe.provide("frappe.utils");
		
		// Store original if it exists
		if (!frappe.utils._original_get_desktop_icon && frappe.utils.get_desktop_icon) {
		...
		frappe.utils.get_desktop_icon = function(icon_name, variant) {
			...
			// Check if boot data exists and app exists
			if (
				!frappe.boot?.desktop_icon_urls ||
				!frappe.boot.desktop_icon_urls[app_name] ||
				!frappe.boot.desktop_icon_urls[app_name][variant]
			) {
				...
				if (
					frappe.boot?.desktop_icon_urls &&
					frappe.boot.desktop_icon_urls[app_name] &&
					frappe.boot.desktop_icon_urls[app_name][fallback_variant]
				) {
					let fallback_url = `assets/${app_name}/icons/desktop_icons/${fallback_variant}/${frappe.scrub(
						icon_name
					)}.svg`;
					let fallback_list = frappe.boot.desktop_icon_urls[app_name][fallback_variant];
